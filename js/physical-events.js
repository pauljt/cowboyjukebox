...
function createSource(instrument) {
    ...
    sources[instrument.imei].env = T("perc", {
        a: 50,
        r: 2500
    });
    sources[instrument.imei].oscenv = T("OscGen", {
        osc: sources[instrument.imei].osc,
        env: sources[instrument.imei].env,
        mul: 0.15
    }).play();

    sources[instrument.imei].tune = T("interval", {
        interval: sources[instrument.imei].interval
    }, function(count) {
...
function modifySource(instrument) {
    ...
    sources[instrument.imei].tune.set({
        'interval': Math.abs(2000 - (~~(dist * 500)))
    })
